...
def do_edge_split(data, val_ratio=0.1, test_ratio=0.1):
    ...
    train_edge = edge_index[:, train_indices]
    val_edge = edge_index[:, val_indices]
    test_edge = edge_index[:, test_indices]

    size = num_nodes * num_nodes
    print(f"Size for negative sampling: {size}")
    print(f"edge_index: {edge_index}")
    print(f"num_nodes: {num_nodes}")
    print(f"num_neg_samples: {num_edges}")

    if size == 0:
        raise ValueError("Size for negative sampling is zero, which is not valid.")

    try:
        alpha = abs(1 / (1 - 1.1 * (edge_index.size(1) / size)))
    except ZeroDivisionError:
        print("Encountered ZeroDivisionError in alpha calculation, adjusting num_neg_samples")
        num_neg_samples = max(1, num_edges // 2)
    else:
        num_neg_samples = num_edges

    if num_neg_samples == 0:
        raise ValueError("Number of negative samples is zero, which is not valid.")
    
    neg_edge = negative_sampling(edge_index, num_nodes=num_nodes, num_neg_samples=num_neg_samples)

    train_neg_edge = neg_edge[:, :train_edge.size(1)]
    val_neg_edge = neg_edge[:, train_edge.size(1):train_edge.size(1) + val_edge.size(1)]
    test_neg_edge = neg_edge[:, train_edge.size(1) + val_edge.size(1):]

    split_edge = {
        'train': {'edge': train_edge, 'edge_neg': train_neg_edge},
        'valid': {'edge': val_edge, 'edge_neg': val_neg_edge},
        'test': {'edge': test_edge, 'edge_neg': test_neg_edge},
    }

    print(f"train_edge shape: {train_edge.shape}")
    print(f"val_edge shape: {val_edge.shape}")
