...
from torch_geometric.utils import to_undirected, add_self_loops, negative_sampling, to_networkx
...
def train_arch(data, split_edge, batch_size, args, save_path_model, save_path_arch, device):
    ...
    for epoch in range(args.epochs):
        ...
        for perm in DataLoader(range(pos_train_edge.size(0)), batch_size, shuffle=True):
            ...
            valid_edge = pos_valid_edge[valid_perm.long()]
            valid_edge_neg = pos_valid_edge_neg[valid_perm.long()]
            h = v_model(data.x, data.edge_index)
        ...
        val_accuracies.append(valid_hits)
...
def visualize_graph(data, edge_index, node_color=None):
    if data.edge_attr is not None and data.edge_attr.size(0) == edge_index.size(1):
    ...
    nx.draw(G, pos, with_labels=True, node_color=node_color, node_size=300, edge_color='gray')
    plt.show()
...
def main(data, args, device):
    ...
    for run in range(args.runs):
        ...
        train_losses, val_accuracies = train_arch(data, split_edge, args.batch_size, args, save_path_model, save_path_arch, device)
    ...
    for run in range(args.runs):
        ...
        plt.subplot(1, 2, 1)
        plt.plot(all_train_losses[run], label='Train Loss')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        ...
        plt.subplot(1, 2, 2)
        plt.plot(all_val_accuracies[run], label='Validation Accuracy')
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy')
    ...
    with open('shop_encoder.pkl', 'wb') as f:
        pickle.dump(shop_encoder, f)
    with open('consumer_encoder.pkl', 'wb') as f:
        pickle.dump(consumer_encoder, f)
    with open('drink_encoder.pkl', 'wb') as f:
        pickle.dump(drink_encoder, f)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Custom Dataset (GNN)')
    parser.add_argument('--device', type=int, default=0)
    parser.add_argument('--use_sage', type=str, default='GCN')
    parser.add_argument('--test_sage', type=str, default='GCN')
    parser.add_argument('--dataset', type=str, default='Custom')
    parser.add_argument('--data_path', type=str, default='Cleaned_Uploaded_Output.csv', help='Path to the custom dataset CSV file')
    ...
    parser.add_argument('--patience', type=int, default=300, help='Use attribute or not')
    ...
    data, shop_encoder, consumer_encoder, drink_encoder = load_custom_data(args.data_path)
    data.edge_index = to_undirected(data.edge_index)
    data = data.to(device)
    split_edge = do_edge_split(data)

    main(data, args, device)
    #visualize_graph(data, data.edge_index)