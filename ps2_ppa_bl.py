...
def train_arch(data, split_edge, batch_size, args, evaluator, save_path_model, save_path_arch, device):
    ...
    pos_valid_edge_neg = torch.randint(0, data.num_nodes, pos_valid_edge.size(), dtype=torch.long,
                                           device=device)
...
def main(data, split_edge, args, device):
    ...
    for run in range(args.runs):
        ...
        save_path_model = 'weight/l3bl' + args.use_sage + '_{}'.format(args.test_sage) + '_{}'.format(
            args.dataset) + '_{}'.format(args.num_layers) + "-lin{}".format(args.lin_layers) + '_lr{}-tem{}'.format(args.lr, args.temperature) \
        ...
        save_path_model_test = 'weight/l3bl' + args.use_sage + '_{}'.format(args.test_sage) + '_{}'.format(
            args.dataset) + '{}'.format(args.num_layers) + "-lin{}".format(args.lin_layers) + '_lr{}-tem{}'.format(args.lr, args.temperature) \
        ...
        t2 = time.time()
        results = train_model(data, split_edge, args.batch_size, args, evaluator, save_path_model_test,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', type=int, default=1024, help='64 * 1024 by default')
    parser.add_argument('--learning_rate_min', type=float, default=0.001, help='min learning rate')
    parser.add_argument('--lr', type=float, default=0.01)
    parser.add_argument('--epochs', type=int, default=20, help='20 by default')
